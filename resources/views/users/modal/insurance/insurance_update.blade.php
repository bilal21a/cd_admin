...
<div class="fv-row mb-5 fv-plugins-icon-container">
    ...
    <input type="date" name="from_date" class="form-control form-control-solid mb-3 mb-lg-0" value="{{$insurance->insurance_date_to ? $insurance->insurance_date_to : date('Y-m-d') }}" required>
    <div class="fv-plugins-message-container invalid-feedback"></div>
    @error('from_date')
        <span class="text-danger">{{ $message }}</span>
    @enderror
...
<div class="fv-row mb-5 fv-plugins-icon-container">
    ...
    <input type="date" name="to_date" class="form-control form-control-solid mb-3 mb-lg-0" value="{{$insurance->insurance_date_from ? $insurance->insurance_date_from : date('Y-m-d') }}" required>
    <div class="fv-plugins-message-container invalid-feedback"></div>
    @error('to_date')
        <span class="text-danger">{{ $message }}</span>
    @enderror
